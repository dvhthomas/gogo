...
func TestPing(t *testing.T) {
	...
	// Here's why we *really* want the application -- it gives us the routes to test
	// This starts up an HTTPS server which listens on a randomly chosen port while
	// the test is running.
	ts := newTestServer(t, app.routes())
	defer ts.Close()

	// ts.URL contains the IP address and port of that temporary server...
	// ...and returns an http.Response struct that we can check.
...
func TestSignupUser(t *testing.T) {
	...
	tests := []struct {
		...
		// Took me a second to figure out why dupe@example.com 'works'. It's because the UserModel.Insert
		// method on the mock has already been called
